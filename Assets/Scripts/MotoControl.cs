...
public class MotoControl : MonoBehaviour {
    ...
    void Start () {
        ...
        radioRueda = GetComponent<CircleCollider2D>().radius; // CAMBIO
    }
    ...
    public void MueveDerecha()
    {
        if (TocaElSuelo())
        {
            myRigidbody.velocity += new Vector2(transform.right.x * velocidadLineal, transform.right.y * velocidadLineal) * Time.deltaTime;
        }
    ...
    {
        myRigidbody.MoveRotation(myRigidbody.rotation - velocidadRotacion * Time.deltaTime);
    }
